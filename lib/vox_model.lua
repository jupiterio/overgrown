-- taken from https://github.com/evgiz/sprit3r/blob/master/src/vox/vox_model.lua (MIT license)
...
function vox_model.new(binaryString)
  ...
  while stream.index < mainChunk.lastBytePos do
    ...
    elseif chunk.id == ID_RGBA then
        ...
        -- modified for love 11
      for i=1, 255 do
        model.palette[i] = { stream:readChar()/255, stream:readChar()/255, stream:readChar()/255, stream:readChar()/255 } -- r,g,b,a
      end
